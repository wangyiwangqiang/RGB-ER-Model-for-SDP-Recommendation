...
cores = multiprocessing.cpu_count() // 2 # multiprocessing.cpu_count() // 2
...
def test_one_user(x,y):
    ...
    user_pos_test =[]
    for i in u:
        for j in test_user_set[i]:
            user_pos_test.append(j)
    # print(user_pos_test)
...
def test(model, user_dict, n_params):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        batch_result = test_one_user(rate_batch, user_list_batch)
